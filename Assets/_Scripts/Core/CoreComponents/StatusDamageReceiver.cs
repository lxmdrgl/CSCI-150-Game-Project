...
namespace Game.CoreSystem
{
    public class StatusDamageReceiver : CoreComponent, IStatusDamageable
    {
        ...
        {
            // test UnityEngine.// test Debug.Log("Status data: " + data);
            // AddStatus(data);
            ...
            if (existingStatus.Item2 == null)
            {
                ...
                // test UnityEngine.// test Debug.Log("Create new Status bar");
            }
            ...
            // test UnityEngine.// test Debug.Log($"Decrease status bar: {stats.Statuses[index].Item1.CurrentValue}, {data.Amount}");

            // If status is 0
            if (stats.Statuses[index].Item1.CurrentValue >= -0.01f && stats.Statuses[index].Item1.CurrentValue <= 0.01f)
            {
                // test UnityEngine.// test Debug.Log($"Reset status bar, adding {data.GetType()}");
        ...
        public void AddStatus(StatusData data)
        {
            ...
            // test UnityEngine.// test Debug.Log("Status effects " + index + ": " + string.Join(", ", statusEffects));
            if (index >= 0)
            {
                // test UnityEngine.// test Debug.Log($"Reapply status: {index} {data.GetType()}");
            ...
            else
            {
                ...
                // test UnityEngine.// test Debug.Log($"Apply status: {index} {data.GetType()}");
        ...
        private void RemoveStatus(StatusData data)
        {
            ...
            {
                // test UnityEngine.// test Debug.Log($"Removed status: {data.GetType().Name}");
