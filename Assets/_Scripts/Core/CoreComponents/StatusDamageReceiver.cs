...
namespace Game.CoreSystem
{
    public class StatusDamageReceiver : CoreComponent, IStatusDamageable
    {
        ...
        {
            // test UnityEngine.// Debug.Log("Status data: " + data);
            // AddStatus(data);
            ...
            if (existingStatus.Item2 == null)
            {
                ...
                // test UnityEngine.// Debug.Log("Create new Status bar");
            }
            ...
            // test UnityEngine.// Debug.Log($"Decrease status bar: {stats.Statuses[index].Item1.CurrentValue}, {data.Amount}");

            // If status is 0
            if (stats.Statuses[index].Item1.CurrentValue >= -0.01f && stats.Statuses[index].Item1.CurrentValue <= 0.01f)
            {
                // test UnityEngine.// Debug.Log($"Reset status bar, adding {data.GetType()}");
        ...
        public void AddStatus(StatusData data)
        {
            ...
            // test UnityEngine.// Debug.Log("Status effects " + index + ": " + string.Join(", ", statusEffects));
            if (index >= 0)
            {
                // test UnityEngine.// Debug.Log($"Reapply status: {index} {data.GetType()}");
            ...
            else
            {
                ...
                // test UnityEngine.// Debug.Log($"Apply status: {index} {data.GetType()}");
        ...
        private void RemoveStatus(StatusData data)
        {
            ...
            {
                // test UnityEngine.// Debug.Log($"Removed status: {data.GetType().Name}");
