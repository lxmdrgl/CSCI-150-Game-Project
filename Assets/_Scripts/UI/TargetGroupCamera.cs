...
public class TargetGroupCamera : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        foreach (var target in targetGroup.Targets)
        {
            ...
            // // test Debug.Log($"Target Camera Rotation Y: {rotationY}");
        ...
        if (rightRotation)
        {
            // // test Debug.Log("Camera Right Rotation");
            transform.rotation = Quaternion.Euler(0, 0, 0);
        }
        else if (leftRotation)
        {
            // // test Debug.Log("Camera Left Rotation");
            transform.rotation = Quaternion.Euler(0, -180, 0);
        } else
        {
            // // test Debug.Log("Camera No Rotation");
