...
namespace Game.Weapons.Components
{
    public class ActionHitBox : WeaponComponent<ActionHitBoxData, AttackActionHitBox>
    {
        ...
        private void HandleAttackAction()
        {
            ...
            hitbox.enabled = true;
            // test Debug.Log("Action hitbox true: " + hitbox.enabled);
            hitbox.points = currentAttackData.HitBox.points;

            // // test Debug.Log($"Weapon points: {currentAttackData.HitBox}");

            Physics2D.OverlapCollider(hitbox, detected);

            // // test Debug.Log($"Detected: {detected}, count: {detected.Count}");

            if (detected.Count == 0)
                return;

            // // test Debug.Log("Charge Hitbox detected: " + detected.Count);
        ...
        protected override void HandleExit()
        {
            ...
            // test Debug.Log("Action hitbox false: " + hitbox.enabled);
        }
