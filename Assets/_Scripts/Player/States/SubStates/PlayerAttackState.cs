...
public class PlayerAttackState : PlayerActionState
{
    ...
    public override void LogicUpdate()
    {
        ...
        {
            // test Debug.Log("Action hitbox on action invoke");
            weapon.EventHandler.OnAttackActionInvoke();
        }

        // // test Debug.Log("checkInterruptable: " + checkInterruptable + " " + inputIndex);
        if (checkInterruptable) {
            if (jumpInput || dashInput) {
                // // test Debug.Log("Interrupt attack: " + string.Join(", ", player.InputHandler.AttackInputs));
                HandleUseInput();
                isActionDone = true;
            }
        }
    ...
    public override void Enter()
    {
        ...
        if (inputIndex == (int)CombatInputs.primarySkillPress || inputIndex == (int)CombatInputs.primarySkillHold) {
            ...
            // test Debug.Log("Dash attack enabled: " + attackEnabled);
        } 
    }
    ...
    public override void Exit()
    {
        ...
        if (inputIndex == (int)CombatInputs.primarySkillPress || inputIndex == (int)CombatInputs.primarySkillHold) {
            ...
            // // test Debug.Log("Start primary skill cooldown: " + weapon.Data.AttackCooldown);
        } 
        else if (inputIndex == (int)CombatInputs.secondarySkillPress || inputIndex == (int)CombatInputs.secondarySkillHold) {
            player.secondarySkillTimeNotifier.Init(weapon.Data.AttackCooldown);
        }

        // test Debug.Log("Action hitbox Weapon exit");
        weapon.Exit();
    }
    ...
    public bool CanAttack(CombatInputs input, CombatInputs output) 
    {
        ...
        if (canAttack)
        {
            // // test Debug.Log("CanAttack: " + string.Join(", ", player.InputHandler.AttackInputs));
            hasInput = player.InputHandler.SimulateAttackInput(input, output);
            /* if (hasInput) 
            {
                // test Debug.Log("HasInput CanAttack: " + string.Join(", ", player.InputHandler.AttackInputs));
            } */
        }
    ...
    public bool CanAttackCooldown(CombatInputs input, CombatInputs output) 
    {
        ...
        // // test Debug.Log(string.Join(", ", player.InputHandler.AttackInputs));
        return canAttack && hasInput;
    }
    ...
    public void DashAttackCooldownDisable() {
        attackEnabled = false;
        // test Debug.Log("Reset dash attack cooldown: " + attackEnabled);   
    }
    ...
    {
        // test Debug.Log("HandleUseInput: " + inputIndex);
